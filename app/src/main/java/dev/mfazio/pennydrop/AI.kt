...
data class AI(val aiId: Long = 0, val name: String, val rollAgain: (slots: List<Slot>) -> Boolean) {
    override fun toString() = name

    fun toPlayer() = Player(playerId = aiId, playerName = name, isHuman = false, selectedAI = this)
    ...
    companion object {
        ...
        val basicAI = listOf(
            AI(
                1,
                "TwoFace"
            ) { slots -> slots.fullSlots() < 3 || (slots.fullSlots() == 3 && coinFlipIsHeads()) },
